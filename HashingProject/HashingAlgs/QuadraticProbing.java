...
public class QuadraticProbing {
    ...
    public void addNode(String value, int n) {
        ...
        while (map[key] != null) {
            wordProbes[probes]++; // Update wordProbes array with the number of probes
            tracker[key]++; // Increment collision tracker
            probes++;
            key = (key + (i * i)) % size; // Quadratic probing
            i++;
        }
    ...
    public void getHashTo() {
        try {
            FileWriter probeWriter = new FileWriter("words_vs_probes_quadraticprobing.txt");
